...
const AI_ENABLED = AI_DEMO_MODE || (process.env.AI_ENABLED !== "false" && Boolean(OPENAI_API_KEY));
...
function createDemoAIReply(message) {
  ...
  if (isArabic) {
    if (/مرحبا|السلام|اهلا|أهلا/.test(text)) return "مرحباً! أنا ConnectChat AI في الوضع التجريبي المجاني. واجهة الذكاء الاصطناعي تعمل بشكل صحيح، لكن إجاباتي حالياً تجريبية وليست من نموذج ChatGPT الحقيقي.";
    if (/تكييف|hvac|fahu|مكيف/.test(lower)) return "الوضع التجريبي يعمل. مثال: نظام HVAC يتحكم بدرجة الحرارة والرطوبة والتهوية داخل المبنى. عند تفعيل API الحقيقي سأستطيع تقديم حسابات وتفاصيل هندسية أدق.";
    if (/ترجم|translation|translate/.test(lower)) return "يمكن اختبار شاشة المحادثة والإرسال والاستقبال الآن. الترجمة الذكية الكاملة ستعمل بعد تفعيل مزود AI الحقيقي.";
    return `تم استلام رسالتك بنجاح: “${text.slice(0, 250)}”\n\nأنا الآن في الوضع التجريبي المجاني. هذا يؤكد أن ConnectChat AI ظاهر وأن مسار الرسائل يعمل. عند تفعيل API الحقيقي سأقدم إجابات ذكية ومفصلة.`;
  }

  if (/hello|hi|hey/.test(lower)) return "Hello! I am ConnectChat AI running in free Demo Mode. The AI contact and message flow are working correctly, but these replies are simulated rather than generated by ChatGPT.";
  if (/hvac|fahu|air condition|mep/.test(lower)) return "Demo Mode is working. Example: HVAC systems control indoor temperature, humidity, ventilation, and air quality. Real engineering calculations and detailed answers will be available after enabling a real AI API.";
  if (/translate|translation/.test(lower)) return "The chat interface is working. Full intelligent translation will be available when a real AI provider is enabled.";
  return `Your message was received successfully: “${text.slice(0, 250)}”\n\nConnectChat AI is currently in free Demo Mode. This confirms the AI contact and messaging workflow work. Real intelligent answers will be available after an AI API is enabled.`;
}
...
app.post("/api/ai/chat", aiLimiter, auth, async (req, res) => {
  try {
    if (!AI_ENABLED) return res.status(503).json({ error: "ConnectChat AI is disabled." });
    const message = cleanText(req.body?.message, 4000);
    if (!message) return res.status(400).json({ error: "Please enter a message." });
    if (AI_DEMO_MODE) {
      await new Promise(resolve => setTimeout(resolve, 500));
      return res.json({ answer: createDemoAIReply(message), model: "demo", demo: true });
    }
