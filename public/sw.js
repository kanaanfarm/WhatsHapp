const CACHE="connectchat-v6791";
const ASSETS=["/","/index.html","/style.css?v=6791","/app.js?v=6791","/manifest.json","/logo.svg"];
...
self.addEventListener("notificationclick",event=>{
  ...
  const target=event.notification.data?.url||"/?v=6791";
...
self.addEventListener("push",event=>{
  ...
  const isCall=data.type==="call";const options={body:data.body||"New notification",icon:"/logo.svg",badge:"/logo.svg",tag:data.tag||"connectchat",renotify:true,requireInteraction:isCall,vibrate:isCall?[300,120,300,120,500]:[120],data:{url:data.url||"/?v=6791"}};
